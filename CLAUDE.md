# CLAUDE.md — Kannan Development Guide

## What is this project?

Kannan is a shell-based multi-agent orchestrator. It coordinates AI coding agents (Claude, Codex, Gemini, Ollama) to collaboratively work on software projects. Written entirely in bash.

## Project Structure

- `kannan` — main entry point, sources all libs, defines CLI commands
- `lib/` — modular shell libraries (each file is sourced, not executed standalone)
  - `util.sh` — shared helpers (gen_id, slugify, state_get/set, kannan_init)
...
- `.kannan/` — runtime state directory (gitignored), contains project_model.json and patches

## Key Conventions

- All lib files are sourced into the main `kannan` script — they share the same shell context
...
- The `KANNAN_DIR` variable (`.kannan`) is used everywhere for state paths

## Testing

```sh
./kannan version          # Smoke test
./kannan agent list       # Check agent discovery
